...
static void sip_recv(struct sip *sip, const struct sip_msg *msg)
{
	...
	if (msg->scode == 0)
	{
		...
		else if (!pl_strcmp(&msg->met, "CANCEL")){
			re_printf("\t CANCEL\n");
		}
		else if (!pl_strcmp(&msg->met, "OPTIONS")){
			re_printf("\t OPTIONS\n");
		}
		else if (!pl_strcmp(&msg->met, "REGISTER")){
			re_printf("\t REGISTER\n");
		}
		else if (!pl_strcmp(&msg->met, "PRACK")){
			re_printf("\t PRACK\n");
		}
		else if (!pl_strcmp(&msg->met, "SUBSCRIBE")){
			re_printf("\t SUBSCRIBE\n");
		}
		else if (!pl_strcmp(&msg->met, "NOTIFY")){
			re_printf("\t NOTIFY\n");
		}
		else if (!pl_strcmp(&msg->met, "PUBLISH")){
			re_printf("\t PUBLISH\n");
		}
...
// 	str = mem_zalloc(mb->size, NULL);
// 	err = mbuf_read_str(mb, str, mb->size);
// 	re_printf("str = %s \n", str);
