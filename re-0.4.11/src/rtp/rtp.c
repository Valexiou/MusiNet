...
struct rtp_sock {
	...
	bool ssrc_mux;          /**< SSRC multiplexing - Multithreading decoding*/
};
...
void * ssrc_mux_thread(void *argument)
{
	struct rtp_header hdr;
	int err;
	struct thread_info* ti = argument;
	
	/* Handle RTCP multiplexed on RTP-port */
	if (ti->rs->rtcp_mux) {
		uint8_t pt;

		if (mbuf_get_left(ti->mb) < 2)
			return NULL;

		pt = mbuf_buf(ti->mb)[1] & 0x7f;

		if (64 <= pt && pt <= 95) {
			rtcp_recv_handler(ti->src, ti->mb, ti->rs);
			return NULL;
		}
	}
	
	err = rtp_decode( ti->rs, ti->mb, &hdr);
	if (err)
		return NULL;
...
// 	}
	...
	if (ti->rs->recvh)
		ti->rs->recvh(ti->src, &hdr, ti->mb, NULL);
...
// The default function was modified to a "my_function" for our scope:
static void udp_recv_handler_ssrc_mux(const struct sa *src, struct mbuf *mb, void *arg)
{
	pthread_t my_thread;
	struct rtp_sock *rs = arg;
	...
	ti = mem_zalloc(sizeof(*ti), NULL);
	ti->rs = rs;
	ti->mb = mbuf_alloc_ref(mb);
	ti->src = src;	
		
	pthread_create(&my_thread, 0, ssrc_mux_thread, ti);
}
			    ...
			    uint16_t min_port, uint16_t max_port)
{
	...
	while (tries--) {
		...
		if (rs->ssrc_mux)
		{
			err = udp_listen(&us_rtp, &rs->local, udp_recv_handler_ssrc_mux, rs);
		}
		else
		{
...
//	/*This code should be enabled, in order to simulate the SSRCs set by our relay server. 
// 	  The goal is to be able to test the capabilities of our system in localhost:*/
// 	if( flag % 2 == 0 )
// 	{
// 		rs->enc.ssrc = 5 + flag; //rand_u32();
// 		flag++;
// 	}	
// 	else if( flag % 2 == 1 )
// 	{
// 		rs->enc.ssrc = 10004 + flag;
// 		flag++;
// 	}
...
// "my_function"
int rtp_listen_ssrc_mux(struct rtp_sock **rsp, int proto, const struct sa *ip,
	       uint16_t min_port, uint16_t max_port, bool enable_rtcp, bool enable_ssrc_mux,
	       rtp_recv_h *recvh, rtcp_recv_h *rtcph, void *arg)
{
	struct rtp_sock *rs;
	int err;
