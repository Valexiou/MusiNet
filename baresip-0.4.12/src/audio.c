...
static void audio_destructor_ssrc_mux(void *arg)
{
	struct audio *a = arg;
...
static int aurx_stream_decode(struct aurx *rx, struct mbuf *mb)
{
	...
	if (!rx->ac)
	{
		//re_printf("Inside aurx_stream_decode : no decoder set\n");
		return 0;
	}

	if (mbuf_get_left(mb)) {
		//re_printf("Inside aurx_stream_decode : if\n");
		err = rx->ac->dech(rx->dec, rx->sampv, &sampc,
				   mbuf_buf(mb), mbuf_get_left(mb));
	}
	else if (rx->ac->plch) {
		//re_printf("Inside aurx_stream_decode : else if\n");
		err = rx->ac->plch(rx->dec, rx->sampv, &sampc);
	}
	else {
		//re_printf("Inside aurx_stream_decode : else\n");
				...
				struct mbuf *mb, void *arg)
{
	...
	//re_printf("Inside stream_recv_handler of audio.c\n");
	...
	if (hdr->pt != rx->pt) {
		//re_printf("Audio payload type changed : rx->pt = %d\n", rx->pt);
...
// // Additional "my_function" for our scope:
// void audio_stream_recv_handler(const struct rtp_header *hdr,
// 				struct mbuf *mb, void *arg)
// {
// 	stream_recv_handler(hdr, mb, arg);
// }
		...
		audio_event_h *eventh, audio_err_h *errh, void *arg)
{
	...
	a->cfg = cfg->audio;
	ssrc_enable_ = cfg->avt.ssrc_mux;
	tx = &a->tx;

	err = tx_stream_alloc(&a->strm, &cfg->avt, call, sdp_sess,
			   "audio", label, mnat, mnat_sess, menc, menc_sess,
			   call_localuri(call));
...
int rx_audio_alloc(struct audio **ap, const struct config *cfg,uint32_t ptime)
{
	...
	a = mem_zalloc(sizeof(*a), audio_destructor_ssrc_mux);
	if (!a)
		return ENOMEM;
	...
	err = rx_stream_alloc(&a->strm, &cfg->avt, stream_recv_handler, NULL, a);
	if (err)
		goto out;
...
int audio_start(struct audio *a)
{
	...
	if (a->cfg.src_first) {
		...
		err |= start_player(&a->rx, a); // rx->ac does not exist, so return 0. The decoding process is not enabled right now.
	}
	else {
		err  = start_player(&a->rx, a); // rx->ac does not exist, so return 0. The decoding process is not enabled right now.
		err |= start_source(&a->tx, a);
	...
	{		
		if( a->tx.ac ){ 
			if (!a->started) {
				info("%H", autx_print_pipeline, &a->tx);
				a->started = true;
			}
		}
	
		if( a->rx.ac ){ 
			info("%H", aurx_print_pipeline, &a->rx);	
		}
	}	
...
{
	stream_sock_cpy(a1->strm, a2->strm);
}
