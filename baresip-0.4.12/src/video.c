...
static void video_destructor_ssrc_mux(void *arg)
{
	struct video *v = arg;
	//struct vtx *vtx = &v->vtx;
	struct vrx *vrx = &v->vrx;
			       ...
			       struct mbuf *mb)
{
	...
	//re_printf("Inside video_stream_decode\n");
	...
	/* No decoder set */
	if (!vrx->dec) {
		re_printf("video: No video decoder!\n");
		goto out;
	}
	...
	if (err) {
		//re_printf("Error calling decode handler\n");
		if (err != EPROTO) {
	...
	{
		//re_printf("Did not get a full picture-frame\n");
		goto out;
	}
	...
	//re_printf("Before vidisp_display\n");
	err = vidisp_display(vrx->vidisp, v->peer, &frame);
	//re_printf("After vidisp_display\n");
...
// 	re_printf("Inside stream_recv_handler of video.c\n");
	
	if (!mb)
		goto out;
		
// 	re_printf("Inside stream_recv_handler of video.c after !mb\n");
...
// // Additional "my_function" for our scope:
// void video_stream_recv_handler(const struct rtp_header *hdr,
// 				struct mbuf *mb, void *arg)
// {
		...
		const char *content, const struct list *vidcodecl)
{
	...
	v->cfg = cfg->video;
	_ssrc_enable = cfg->avt.ssrc_mux;
	tmr_init(&v->tmr);

	err = tx_stream_alloc(&v->strm, &cfg->avt, call, sdp_sess, "video", label,
			   mnat, mnat_sess, menc, menc_sess, call_localuri(call));
	if (err)
		goto out;
...
int rx_video_alloc(struct video **vp, const struct config *cfg)
{
	...
	v = mem_zalloc(sizeof(*v), video_destructor_ssrc_mux);
	if (!v)
		return ENOMEM;
	...
	err = rx_stream_alloc(&v->strm, &cfg->avt, stream_recv_handler, rtcp_handler, v);
	if (err)
		goto out;
...
{
	stream_sock_cpy(v1->strm, v2->strm);
