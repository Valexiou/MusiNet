	       ...
	       call_event_h *eh, void *arg)
{
	...
	if (msg && mbuf_get_left(msg->mb)) // Does not enter here:
		got_offer = true;
	...
	if ( cfg->avt.ssrc_mux )
	{
		err = tx_audio_alloc(&call->audio, cfg, call,
				  call->sdp, ++label,
	...
	if (use_video) {
		if ( cfg->avt.ssrc_mux )
		{
			err = tx_video_alloc(&call->video, cfg,  
					  call, call->sdp, ++label,
					  acc->mnat, call->mnats,
					  acc->menc, call->mencs,
					  "main",
...
void vrx_update(struct call *call, struct video **v)
{
	...
	video_decoder_set(*v, sc->data, sc->pt, sc->rparams);
	
	rx_set_vidisp(*v);
	
	vtx_sock_cpy(*v, call->video);
}
...
void arx_update(struct call *call, struct audio **a)
{
	...
	audio_decoder_set(*a, sc->data, sc->pt, sc->rparams);
	
	atx_sock_cpy(*a, call->audio);
}
