...
uint32_t max_assrc = 0;
uint32_t max_vssrc = 0;
...
static void user_list_destructor(void)
{
	...
	while( temp->next != NULL)
	{
		...
		pthread_mutex_destroy(&temp->abufmtx);
		pthread_mutex_destroy(&temp->abufmtx);
		
		pthread_cond_destroy(&temp->abufprocessing);
		pthread_cond_destroy(&temp->vbufprocessing);
	...
	re_printf("\nAll resources were cleaned successfully !!!\n\n");
}
...
int init_list( struct node **f, struct node **l, uint32_t ssrc, int packet_type)
{
	...
	if( (temp = mem_zalloc(sizeof(*temp), NULL)) == NULL)
	{
		re_printf("Error init_list() : mem_zalloc temp\n");
		return -1;
	}
	
	if( packet_type == 0 )
	{
		max_assrc = temp->a_ssrc = ssrc;
		max_vssrc = temp->v_ssrc = ssrc + 10000;		
	}
	else
	{
		max_assrc = temp->a_ssrc = ssrc - 10000;
		max_vssrc = temp->v_ssrc = ssrc;
	}
	
	rx_audio_alloc(&(temp->a), conf_config(), uag.ua_cur->acc->ptime);
	rx_video_alloc(&(temp->v), conf_config());
	
	vrx_update(ua_call(uag.ua_cur), &temp->v);
	arx_update(ua_call(uag.ua_cur), &temp->a);
	
	pthread_mutex_init(&temp->abufmtx, NULL);
	pthread_mutex_init(&temp->vbufmtx, NULL);
	
	pthread_cond_init(&temp->abufprocessing, NULL);
	pthread_cond_init(&temp->vbufprocessing, NULL);	
...
void *insert_node(uint32_t ssrc, int packet_type)
{
 	...
 	if( packet_type == 0 )	// audio packet
 	{
		if (first == NULL)
		{
			init_list(&first, &last, ssrc, packet_type);
		}
		else
		{
			create_node(&last, ssrc, packet_type);
		}
...
void *find_user(uint32_t ssrc, int packet_type)
{
	...
	if ( temp == NULL)
	{
		re_printf("There is no user yet!\n");
		return NULL;
	}
